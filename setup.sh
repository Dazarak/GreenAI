...
if ! command -v python3 &> /dev/null; then
    echo "Python3 n'est pas installé." exit 1
fi
...
if [ ! -f "API_KEY.txt" ]; then
    read -p "Entrez la clé API pour sécuriser le serveur (ou Entrée pour en générer une aléatoire) : " USER_KEY
    if [ -z "$USER_KEY" ]; then
...
if [ -d "api_server" ]; then
    ...
    echo "Dossier api_server introuvable !"
fi

echo "=== Installation terminée avec succès ! ==="
echo "Pour lancer le moteur Python : source ai_engine/GreenAI/bin/activate && python ai_engine/main.py"
echo "Pour lancer le serveur API   : cd api_server && npx ts-node src/index.ts"